...
Vagrant.configure("2") do |config|
  config.vm.define "master" do |m|
  	m.vm.box = "ubuntu/bionic64"
        ...
        m.vm.network "private_network", ip: "10.11.13.3", :name => 'vboxnet0'
        m.vm.network :forwarded_port, guest: 8080, host: 8000
        m.vm.network :forwarded_port, guest: 80, host: 8001, guest_ip: "10.11.13.3"
  ...
  (1..2).each do |i|
   config.vm.define "node#{i}" do |m|
        ...
        m.vm.network "private_network", ip: "10.11.13.#{i+3}", :name => 'vboxnet0'
        m.vm.provider :virtualbox do |vb|
		vb.customize [ 'modifyvm', :id, '--memory', '750' ]
		vb.customize [ 'modifyvm', :id, '--cpus', '1' ]
		vb.customize [ 'modifyvm', :id, '--name', "cluster-node-#{i}" ]
  	end
        m.vm.provision "shell", path: "node.sh"
        m.vm.provision "shell", inline: "sudo  sed -i -e 's/W/W#{i}/g' /var/www/html/index.html"
       end
   end
end
